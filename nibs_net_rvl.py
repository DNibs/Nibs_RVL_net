...
class NibsNetRVL:
    ...
    def __init__(self):
        ...
        self.nibs_net_rvl = nn.NeuralNetwork([3, 27, 3], no_cuda=self.no_cuda)

    # forward takes in one image (as a 28*28 tensor) and predicts class
    def forward(self, in_img):

        # reshape image as a 1*784
        in_img_1d = in_img.view(1, 28*28).to(self.device)
    ...
    def train(self):
        ...
        for i in range(0, self.epochsN):
            ...
            for j in range(0, len_data):
                ...
                target_onehot = target_example[j].to(self.device)
            ...
            train_error[i] = self.nibs_net_rvl.getError() / self.batch_size
